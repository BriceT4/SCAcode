...
def solver(inp, infile_name):
    ...
    def interpPropTable(YourProp, YourValue):
        ...
        for i in range(len(proptable)):
            # if value in question is in proptable
            # i.e., it matches up perfectly
            if proptable.loc[i, YourProp] == YourValue:
                # then just use the props given, no need to interpolate 
                InterpValues = pd.DataFrame(proptable.loc[i, :])
                # transpose so property is column header,
                # instead of index (first row)
                InterpValues = InterpValues.T
                # otherwise, we need to interpolate, so we must find the values in 
                # proptable that are directly above and below our provided value 
            elif proptable.loc[i, YourProp] < YourValue and proptable.loc[i+1, YourProp] > YourValue:
                for prop in proptable.columns:
                    interp = scipy.interpolate.interp1d(
                                                        # these are x values, the values directly
                                                        # above and below the value you provided
                                                        [proptable.loc[i, YourProp],
                                                        proptable.loc[i+1, YourProp]
                                                        ],
                                                        # these are the y values to be
                                                        # interpolated between.
                                                        [proptable.loc[i][prop],
                                                        proptable.loc[i+1][prop]
                                                        ]
                                                        )
                    prop_interp = interp(YourValue)
                    InterpValues.append({prop:prop_interp})
                InterpValues = pd.DataFrame(InterpValues)
                # remove NaN's generated by the nature of the loop used to interpolate
                InterpValues = InterpValues.apply(lambda x: pd.Series(x.dropna().values))
                InterpValues.columns = proptable_header
        # calculate densities by inverting specific volumes
        rhos = pd.DataFrame({'rho_l (kg/m^3)':InterpValues.loc[:, 'vol_l (m^3/kg)']**(-1),
                            'rho_g (kg/m^3)':InterpValues.loc[:, 'vol_g (m^3/kg)']**(-1)
                            })
        # add densities to InterpValues
        InterpValues = pd.concat([InterpValues, rhos])
        # remove NaN's again (the ones that were generted in the "rhos" calculation).
        InterpValues = InterpValues.apply(lambda x: pd.Series(x.dropna().values))
